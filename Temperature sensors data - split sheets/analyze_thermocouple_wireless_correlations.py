...
OUTPUT_DIR = BASE_DIR / "thermocouple_wireless_results"
...
GROUPS = [
    {
        "key": "30C",
        "title": "30C Thermocouple vs Wireless Sensor 1",
    ...
    {
        "key": "37C",
        "title": "37C Thermocouple vs Wireless Sensor 2",
...
def load_group_data(group):
    wireless = load_wireless(group["wireless_file"], group["wireless_name"], group["sensor_cols"])
    thermocouple = load_thermocouple(group["thermocouple_file"], group["thermocouple_name"])
    return merge_by_nearest_time(wireless, thermocouple)
...
def plot_correlation(df, group):
    corr = df[[group["thermocouple_name"], group["wireless_name"]]].corr()
    fig, ax = plt.subplots(figsize=(7, 5))
    image = ax.imshow(corr, vmin=-1, vmax=1, cmap="coolwarm")
    labels = [group["thermocouple_name"], group["wireless_name"]]
    ...
    fig.tight_layout()
    path = OUTPUT_DIR / f"{group['key']}_thermocouple_wireless_correlation.png"
...
def plot_time_series(df, group):
    ...
    fig.tight_layout()
    path = OUTPUT_DIR / f"{group['key']}_thermocouple_wireless_trends.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def plot_model_results(group, metrics_rows, predictions):
    ...
    fig.suptitle(
        f"{group['title']} - Predicting Wireless Sensor from Thermocouple",
        fontsize=14,
    )
    ...
    for ax, model_name in zip(axes.ravel(), model_names):
        ...
        ax.set_xlabel("Actual wireless temperature (C)")
        ax.set_ylabel("Predicted wireless temperature (C)")
        ax.grid(True, alpha=0.25)
    ...
    path = OUTPUT_DIR / f"{group['key']}_thermocouple_wireless_model_results.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
...
def run_group(group):
    ...
    df.to_csv(OUTPUT_DIR / f"{group['key']}_thermocouple_wireless_aligned_data.csv", index=False)

    x = df[[group["thermocouple_name"]]]
    y = df[group["wireless_name"]]
    ...
    metrics_frame.to_csv(OUTPUT_DIR / f"{group['key']}_thermocouple_wireless_model_metrics.csv")
